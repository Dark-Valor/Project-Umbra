...
public class NPC : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            speed = other.gameObject.GetComponent<Player_Move_Update>().playerSpeed;

            //Messages to display for main dialogue
            if (other.gameObject.tag == "Player" && notHit)
            {
                for (int i = 0; i < mainDialogueMessages.Length; ++i)
                { //use a queue structure for sequential messages
    ...
    IEnumerator displayText(Collider2D other)
    {
        ...
        { //main loop will go through all the messages in the queue
            ...
            if (useCharDelay)
            {
                ...
                { //sequentially add each character in the message to the text
                    message.text += s[i];
            ...
            { //wait for input loop
                if (useCIFlash && Time.time > nextFlip)
                { //flashing effect, indicator flips between its color and clear every intervl
        ...
        other.gameObject.GetComponent<Player_Move_Update>().playerSpeed = speed;
        notHit = false;
    }
    ...
    void OnTriggerExit2D(Collider2D other)
    {
        if (other.gameObject.tag == "Player")
        {
            message.text = "";
            if (useCI)
            { //if you had a continue indicator, this will turn it off upon leaving
                continueIndicator.fillAmount = 0f;
...
/*  old NPC.cs from original scripts
